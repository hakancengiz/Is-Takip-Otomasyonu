...
<font size="-1" color="#CC3">Not: Sadece kendi biriminizin iş kaydını silebilirsiniz.</font></div>
<?php
...
?>
    ...
    <?php
		$bul = $yaz['sorumlu_personel'];
		$sorgu=mysql_query("select sorumlu_mudur from tbl_koordinator where id = (Select sorumlu_koordinator from tbl_personel where id=$bul)");
		while($incele=mysql_fetch_array($sorgu))
		{
			$isin_sorumlu_muduru = $incele['sorumlu_mudur']; 
		}
		if($isin_sorumlu_muduru == $id)
		{
	?>
