...
<div class="menu">
<?php
$mudur_eposta = $_SESSION['muduroturum'];

$uye_list=mysql_query("Select * from tbl_mudur where eposta='$mudur_eposta'");
...
<font size="-1" color="#CC3">Not: Maaşınızı ve bağlı olduğunuz kurumu değiştirme yetkiniz yok.</font></div>
<font color="#FFFFFF">
...
<tr>
<td>Kurum ID:</td><td><input type="text" name="kurum" class="textbox"  value="<?php echo $yaz['kurum']; ?>" disabled /></td>
</tr>
<tr>
...
if($btn)
{
	...
	$hedef="../resimler/mudur/".$_FILES["resim"]['name'];
	if(is_uploaded_file($fotograf))
	{
		...
		if(!(empty($adsoyad)) && !(empty($eposta)) && !(empty($parola)))
		{
			...
			mysql_query("update tbl_mudur set ad_soyad='$adsoyad', fotograf='$hedef', eposta='$eposta', sifre='$parola' where eposta='$mudur_eposta'")
			or 
