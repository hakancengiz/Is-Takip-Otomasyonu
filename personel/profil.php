...
<div class="menu">
<?php
$personel_eposta = $_SESSION['personeloturum'];

$uye_list=mysql_query("Select * from tbl_personel where eposta='$personel_eposta'");
...
<tr>
<td>Sorumlu Kor. ID:</td><td><input type="text" name="mudur" class="textbox"  value="<?php echo $yaz['sorumlu_koordinator']; ?>" disabled /></td>
</tr>
<tr>
...
if($btn)
{
	...
	$hedef="../resimler/personel/".$_FILES["resim"]['name'];
	if(is_uploaded_file($fotograf))
	{
		...
		if(!(empty($adsoyad)) && !(empty($eposta)) && !(empty($parola)))
		{
			...
			mysql_query("update tbl_personel set ad_soyad='$adsoyad', fotograf='$hedef', eposta='$eposta', sifre='$parola' where eposta='$personel_eposta'")
			or 
